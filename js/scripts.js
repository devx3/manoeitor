...
function Roda()//A cada tecla precionada, irá rodar essa função, usando de regex para converter seus valores
{
  ...
  //t = t.replace(/\bent[ãa]o\b/gi,   			  	  'e daí'); //então olha...
	...
	t = t.replace(/\bbicicleta\b/gi,     	 		'magrela'); //Ele levou minha bicicleta...
	t = t.replace(/\bmenino\b/gi,   	 			  'doido'); //Quer comprar uma bicicleta menino?...
	t = t.replace(/\bquer\b/gi,		   	 			  	'qué'); //Quer comprar uma bicicleta menino?...
	t = t.replace(/\bcomprar\b/gi,		   	 		 'comprá'); //Quer comprar uma bicicleta menino?...
	t = t.replace(/\b(dar umas voltas|passear)\b/gi, 'dar um role'); //Vamos dar umas voltas...

	
	//Nomes
	t = t.replace(/\bjorge\b/gi, 					 'Jorjão'); //Meu amigo Jorge...
